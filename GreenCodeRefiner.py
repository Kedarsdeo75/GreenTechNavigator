...
# Load environment variables for code conversion
env_path = os.path.abspath(".env")
load_dotenv(dotenv_path=env_path, verbose=True, override=True)

# Obtain src and GreenCode directories for conversion 
...
# Remove GreenCode directory from working folder 
...
# Ensure temp and TestCases directories exist before conversion
...
# If testcases does not exists, create unit test cases   
...
# Load prompts from the .env separate file
...
# Identify list of files for conversion
file_list = list(identify_source_files(source_directory, file_extensions, excluded_files))

# Step 1: Create unit test cases for each function as it is required for dynamic code carbon analysis
create_unit_test_files(file_list)

# Update list of files to include newly created test files containing testcases
file_list = list(identify_source_files(source_directory, file_extensions, excluded_files))

# Convert files using Azure OpenAI API one by one
while file_list:
    ...
    # Apply only the prompts from .env files which are marked as "Yes"
